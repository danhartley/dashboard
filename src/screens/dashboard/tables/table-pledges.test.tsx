...
import { createWrapper, renderPledgesComponent, renderPledgesWithSuccess } from "src/screens/dashboard/tables/shared/test-helpers";
    ...
    describe("has a pledge column", () => {
      ...
      describe("which when clicked", () => {
        test("shows the related pledge checklsit in a new row", async () => {
          ...
          renderPledgesComponent();
          await renderPledgesWithSuccess("RTW", 1);
          const button = screen.getByRole("button", { name: /We pledge that all decisions will be explainable, so that they can be verified./i });
          ...
          const row = screen.getByText(/We pledge that all decisions will be explainable, so that they can be verified./i).closest("tr");
          expect(within(row).getByText(/We pledge that all decisions will be explainable, so that they can be verified./i)).toBeTruthy();          
        });
      });
    });  
});